...
- **BLUB issuer `home_domain` MUST stay `whalehub.io`** (set 2026-07-13, tx `d5082fc33b61`). Wallets/explorers verify BLUB via `home_domain` → `https://whalehub.io/.well-known/stellar.toml` (has the BLUB `[[CURRENCIES]]` entry + icon, served text/plain + CORS `*`). **Do NOT set it to lobstr.co or anything else.** ⚠️ **RECURRING REVERT:** it keeps flipping back to `lobstr.co` (06-11 set to app.whalehub.io → 07-11 reverted to lobstr.co, tx `8d0c42d0c6be`, manager key → 07-13 re-set to whalehub.io). Something automated (LOBSTR listing flow? a backend/script?) re-sets it — **root cause UNRESOLVED; find & stop it.** A stale "lobstr.co" in a wallet is cache lag, not necessarily an on-chain revert — confirm via Horizon `home_domain` before re-fixing. toml lives in `website-redesign/.well-known/stellar.toml`; re-point via `stellar tx new set-options --source-account blub-issuer-v2 --home-domain whalehub.io ...`.
